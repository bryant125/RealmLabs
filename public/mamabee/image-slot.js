...
(() => {
  ...
  // sidecar enormous. A 2000px WebP at q=0.85 is ~300-600KB.
  const MAX_DIM = 2000;
  // Floor on the encoded longest side so small slots (e.g. a ~280px phone
  // mockup screen) still store a retina-sharp image instead of a soft
  // upscaled thumbnail.
  const MIN_DIM = 1290;
  ...
  async function toDataUrl(file, targetW) {
    ...
    try {
      const cap = Math.min(MAX_DIM, Math.max(MIN_DIM, Math.round(targetW * 2)) || MAX_DIM);
