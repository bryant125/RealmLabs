...
(function () {
  ...
  L('pv2.providers.note', [
    '我们运营应用所依赖的每一家公司——云托管、AI 处理、订阅、广告衡量——都受书面数据处理协议的约束，只能按我们的指示处理你的数据，对其保密并保障安全，绝不将其用于自身目的。它们不得出售你的数据，我们也绝不向任何人出售。',
    '我們營運應用所依賴的每一家公司——雲託管、AI 處理、訂閱、廣告衡量——都受書面資料處理協議的約束，只能按我們的指示處理你的資料，對其保密並保障安全，絕不將其用於自身目的。它們不得出售你的資料，我們也絕不向任何人出售。',
    'アプリを運営するために私たちが頼るすべての企業——クラウドホスティング、AI 処理、サブスクリプション、広告測定——は、書面によるデータ処理契約に拘束され、あなたのデータを私たちの指示に従ってのみ扱い、機密として安全に保ち、自社の目的には決して使用しません。これらの企業があなたのデータを販売することはできず、私たちも誰にも販売しません。',
    '앱을 운영하기 위해 우리가 의존하는 모든 회사 — 클라우드 호스팅, AI 처리, 구독, 광고 측정 — 는 서면 데이터 처리 계약에 따라 당신의 데이터를 오직 우리의 지시에 따라서만 처리하고, 기밀로 안전하게 유지하며, 결코 자신의 목적으로 사용하지 않도록 구속됩니다. 그들은 당신의 데이터를 판매할 수 없으며, 우리도 누구에게도 판매하지 않습니다.',
    'Jedes Unternehmen, auf das wir angewiesen sind, um die App zu betreiben — Cloud-Hosting, KI-Verarbeitung, Abonnements, Werbemessung — ist durch einen schriftlichen Auftragsverarbeitungsvertrag verpflichtet, Ihre Daten nur nach unseren Weisungen zu verarbeiten, sie vertraulich und sicher zu behandeln und niemals für eigene Zwecke zu nutzen. Sie dürfen Ihre Daten nicht verkaufen, und wir verkaufen sie niemals an irgendjemanden.',
    'Cada empresa de la que dependemos para hacer funcionar la app — alojamiento en la nube, procesamiento con IA, suscripciones, medición de anuncios — está obligada por un acuerdo escrito de tratamiento de datos a manejar tus datos únicamente según nuestras instrucciones, mantenerlos confidenciales y seguros, y no usarlos nunca para sus propios fines. No pueden vender tus datos, y nosotros nunca los vendemos a nadie.',
    'Chaque entreprise dont nous dépendons pour faire fonctionner l\'app — hébergement cloud, traitement par IA, abonnements, mesure publicitaire — est tenue par un accord écrit de traitement des données de ne traiter vos données que selon nos instructions, de les garder confidentielles et sécurisées, et de ne jamais les utiliser à ses propres fins. Elles ne peuvent pas vendre vos données, et nous ne les vendons jamais à quiconque.',
    'Ogni azienda su cui facciamo affidamento per far funzionare l\'app — hosting cloud, elaborazione con IA, abbonamenti, misurazione degli annunci — è vincolata da un accordo scritto sul trattamento dei dati a trattare i tuoi dati solo secondo le nostre istruzioni, a mantenerli riservati e sicuri e a non usarli mai per fini propri. Non possono vendere i tuoi dati, e noi non li vendiamo mai a nessuno.',
    'Каждая компания, на которую мы полагаемся для работы приложения, — облачный хостинг, ИИ-обработка, подписки, измерение рекламы — обязана по письменному соглашению об обработке данных обрабатывать ваши данные только по нашим указаниям, хранить их конфиденциально и безопасно и никогда не использовать в собственных целях. Они не могут продавать ваши данные, а мы никогда не продаём их никому.',
    'ทุกบริษัทที่เราพึ่งพาในการให้แอปทำงาน — โฮสติ้งบนคลาวด์ การประมวลผลด้วย AI การสมัครสมาชิก การวัดผลโฆษณา — ผูกพันตามข้อตกลงการประมวลผลข้อมูลเป็นลายลักษณ์อักษรที่ให้จัดการข้อมูลของคุณตามคำสั่งของเราเท่านั้น เก็บเป็นความลับและปลอดภัย และไม่นำไปใช้เพื่อวัตถุประสงค์ของตนเองเด็ดขาด พวกเขาจะขายข้อมูลของคุณไม่ได้ และเราก็ไม่เคยขายให้ใครทั้งสิ้น',
    'Mọi công ty mà chúng tôi dựa vào để vận hành ứng dụng — lưu trữ đám mây, xử lý AI, đăng ký, đo lường quảng cáo — đều bị ràng buộc bởi một thỏa thuận xử lý dữ liệu bằng văn bản, chỉ được xử lý dữ liệu của bạn theo chỉ dẫn của chúng tôi, giữ bí mật và an toàn, và không bao giờ sử dụng cho mục đích riêng của họ. Họ không được bán dữ liệu của bạn, và chúng tôi không bao giờ bán nó cho bất kỳ ai.'
  ]);
  L('pv2.lives.3', [
    '退出共享家庭即会停止同步。',
    '退出共享家庭即會停止同步。',
